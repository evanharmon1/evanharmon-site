...
if (!MODES.has(mode)) {
  console.error(`agent-registry-labels: mode must be one of ${[...MODES].join(', ')} (got ${mode ?? '<none>'})`);
  process.exit(2);
}
...
try {
  ...
  console.error(`agent-registry-labels: cannot read valid JSON from ${registryPath}: ${error.message}`);
  process.exit(1);
}
...
if (mode === 'suggest-claim' || mode === 'all') {
  for (const family of registry.families ?? []) {
    ...
    lines.push(record(labelName('suggest', slug), COLOR_SUGGEST, `Suggested for the ${name} family (advisory)`));
...
if (mode === 'foreman-adapters' || mode === 'all') {
  for (const adapter of registry.foreman_adapters ?? []) {
    ...
    lines.push(
      record(labelName('foreman', slug), COLOR_FOREMAN, `Arm this issue for foreman dispatch with the ${name} backend`)
    );
  }
}
...
if (mode === 'docs-tables') {
  ...
  for (const harness of registry.harnesses ?? []) {
    ...
    if (constraint.kind === 'fixed') {
      ...
      const defaultFamily = code(cell(constraint.default_family, `harness '${slug}' family_constraint.default_family`));
