...
const schemaPath = path.resolve(process.argv[3] ?? path.join(path.dirname(registryPath), 'agent-registry.schema.json'));
...
const supportedInstanceTypes = new Set(['array', 'boolean', 'integer', 'null', 'number', 'object', 'string']);
...
function assertSchemaKeywordValues(rule, location) {
  ...
  if (Object.hasOwn(rule, 'enum')) {
    ...
    if (
      rule.enum.some((candidate, index) => rule.enum.slice(0, index).some((earlier) => jsonEqual(candidate, earlier)))
    ) {
  ...
  if (Object.hasOwn(rule, 'minLength') && Object.hasOwn(rule, 'maxLength') && rule.maxLength < rule.minLength) {
  ...
  if (Object.hasOwn(rule, 'additionalProperties') && typeof rule.additionalProperties !== 'boolean') {
    schemaError(location, 'additionalProperties', 'must be a boolean');
  }
}

function assertSupportedSchema(rule, location = '$schema', audit = { active: new Set(), complete: new Set() }) {
  ...
  if (Object.hasOwn(rule, '$ref')) {
    ...
    if (!isSchemaObject(target)) {
      throw new Error(`${location}: schema reference ${rule.$ref} does not resolve to an object schema`);
    }
...
if (errors.length === 0) {
  ...
  for (const slug of duplicateSlugs(registry.families)) semanticError(`duplicate family slug: ${slug}`);
  for (const slug of duplicateSlugs(registry.harnesses)) semanticError(`duplicate harness slug: ${slug}`);
  ...
  for (const harness of registry.harnesses) {
    ...
    if (harness.provider_rewired) {
      ...
      if (constraint.kind !== 'fixed' || (harness.slug !== expected && harness.slug !== `${expected}-local`)) {
        semanticError(
      ...
      if (harness.model_resolution.owner !== 'provider-wrapper') {
        semanticError(`provider-rewired harness ${harness.slug} must delegate model resolution to provider-wrapper`);
      }
    } else if (harness.model_resolution.owner === 'provider-wrapper') {
      semanticError(`non-rewired harness ${harness.slug} cannot delegate model resolution to provider-wrapper`);
    }
  }
  ...
  for (const adapter of registry.foreman_adapters) {
    ...
    if (adapter.production_dispatchable) {
      if (adapter.classification !== 'production' || adapter.harness === null) {
        semanticError(`production-dispatchable Foreman adapter ${adapter.slug} needs a production harness mapping`);
      }
      if (!adapter.provision_label) {
        semanticError(`production-dispatchable Foreman adapter ${adapter.slug} must provision its selector label`);
      }
    }
    if (adapter.classification === 'test-only') {
      if (adapter.production_dispatchable || adapter.provision_label) {
        semanticError(`test-only Foreman adapter ${adapter.slug} cannot dispatch or provision a public label`);
      }
    }
    if (adapter.provision_label && !adapter.production_dispatchable) {
      semanticError(`Foreman adapter ${adapter.slug} cannot provision a label unless it is production-dispatchable`);
    }
  }
  ...
  if (!claude || claude.classification !== 'production' || !claude.production_dispatchable || !claude.provision_label) {
  ...
  if (
    ...
    semanticError('MiniMax must use family minimax and provider-rewired harness claude-code-minimax');
  }
}
