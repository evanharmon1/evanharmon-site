...
const schemaPath = path.resolve(process.argv[3] ?? path.join(path.dirname(registryPath), 'label-registry.schema.json'));
...
if (errors.length === 0) {
  ...
  for (const family of registry.families) {
    ...
    if (!retired && family.writers.length === 0) {
      semanticError(`${where}: a live family needs at least one writer (only retired may have none)`);
    }
    ...
    for (const value of family.values) {
      ...
      if ([...name].length > GH_LABEL_NAME_MAX) {
        semanticError(`${at}: composed name '${name}' exceeds GitHub's ${GH_LABEL_NAME_MAX}-char limit`);
      }
      ...
      if (provisioned) {
        if (!value.description) {
          semanticError(`${at}: provisioned labels need a description (GitHub shows it at apply time)`);
        }
        ...
        if (provisionedNames.has(name)) {
          semanticError(`${at}: label '${name}' is already provisioned by family ${provisionedNames.get(name)}`);
        }
        provisionedNames.set(name, family.family);
      }
    }
    if (familyArms && family.prefix !== 'foreman' && family.values.length === 0) {
      semanticError(`${where}: arming outside the foreman:* namespace — foreman:* is the only arming surface`);
    }
  }
}
