    ...
    switch (rest[i]) {
      ...
      case '--pass': {
        ...
        const data = loadAndValidateContext(passFile, '--pass', (candidate) => {
          ...
          if (candidate && typeof candidate.role === 'string' && !passAllowedRoles.includes(candidate.role)) {
            return [`$result.role: expected ${passAllowedRoles.join(' or ')}, found ${JSON.stringify(candidate.role)}`];
          }
          ...
          if (errors.length === 0 && candidate.status === 'blocked') {
            const hasFindings =
              candidate.payload && Array.isArray(candidate.payload.findings) && candidate.payload.findings.length > 0;
...
function checkFindingIds(envelope, options, errors) {
  ...
  for (const finding of payload.findings) {
    ...
    if (Array.isArray(options.knownIds) && options.knownIds.includes(finding.id)) {
      errors.push(`$result.payload.findings: finding id ${finding.id} collides with a finding already in the run`);
    }
...
function checkChallengerAttackScenarios(envelope, errors) {
  ...
  const findingIds = new Set(
    Array.isArray(payload.findings) ? payload.findings.filter((f) => typeof f.id === 'string').map((f) => f.id) : []
  );
  const seenScenarioIds = new Set();
  for (const scenario of payload.attack_scenarios) {
    if (typeof scenario.id === 'string') {
      if (seenScenarioIds.has(scenario.id)) {
        errors.push(`$result.payload.attack_scenarios: duplicate scenario id ${scenario.id} within this pass`);
      }
...
function checkHeadAgreement(kind, envelope, errors) {
  ...
  if (
    ...
    errors.push(`$result.payload.reviewed_head: ${payload.reviewed_head} does not match envelope head ${head}`);
  }
  if (kind === 'integrator' && payload.codex_cycle && typeof payload.codex_cycle === 'object') {
    if (typeof payload.codex_cycle.head === 'string' && payload.codex_cycle.head !== head) {
      errors.push(`$result.payload.codex_cycle.head: ${payload.codex_cycle.head} does not match envelope head ${head}`);
    }
...
function checkRunChronology(document, errors) {
  ...
  if ((document.settlements ?? []).length > 0 && !firstIntegrationEntry) {
    errors.push('$run.settlements: must be empty until stage_transitions records the integration stage');
  }
...
function checkEvidenceMarkerPrDestinationRequiresPr(document, errors) {
  ...
  for (const [index, comment] of document.evidence_comments.entries()) {
    const marker = comment.marker;
    if (marker && marker.destination === 'pr' && (document.pr === null || document.pr === undefined)) {
      errors.push(
...
function checkAdjudicationEvidenceMarkers(document, adjudications, errors) {
  ...
  for (const { file, data } of adjudications) {
    const hasMarker = markers.some((marker) => marker.stage === data.stage && marker.round === data.round);
    if (!hasMarker) {
      errors.push(
...
function checkDeferredFindingsSettledBeforePromotion(document, adjudications, errors) {
  ...
  const settledIds = new Set(
    (document.settlements ?? []).map((entry) => entry.finding_id).filter((id) => typeof id === 'string')
  );
