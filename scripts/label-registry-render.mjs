...
if (!MODES.has(mode)) {
  console.error(`label-registry-render: mode must be one of ${[...MODES].join(', ')} (got ${mode ?? '<none>'})`);
  process.exit(2);
}
...
try {
  ...
  console.error(`label-registry-render: cannot read valid JSON from ${manifestPath}: ${error.message}`);
  process.exit(1);
}
...
function registryFamilyRecords(family) {
  const rendererMode = family.registry_set === 'foreman-adapters' ? 'foreman-adapters' : 'suggest-claim';
...
const provisioned = (family) => family.provision === true && family.retired !== true && gateOpen(family);
...
if (mode === 'labels') {
  for (const family of manifest.families ?? []) {
    ...
    for (const value of family.values ?? []) {
      ...
      lines.push(record(name, value.color ?? family.color, value.description ?? '', `family ${family.family}`));
    }
  }
...
if (mode === 'inventory') {
  ...
  for (const family of manifest.families ?? []) {
    if (family.retired === true || family.source !== 'agent-registry' || family.prefix === null) continue;
...
if (mode === 'docs-table') {
  ...
  const writersText = (writers) =>
    writers.map((writer) => WRITER_TEXT[writer] ?? writer.replace(/^tool:/, '') + ' (tool)').join(' or ');
  const writerCell = (family, value) =>
    value?.writer_note ??
    (value?.writers ? writersText(value.writers) : (family.writer_note ?? writersText(family.writers)));
  ...
  const lifecycleCell = (family, value) =>
    ...
    (value?.lifecycle ? LIFECYCLE_TEXT[value.lifecycle] : (family.lifecycle_note ?? LIFECYCLE_TEXT[family.lifecycle]));
  ...
  for (const family of manifest.families ?? []) {
    ...
    if (family.source === 'agent-registry' || (family.values.length === 0 && family.open_values)) {
      const label =
        code(cell(composedName(family, family.placeholder), `family ${family.family} placeholder`)) + retiredSuffix;
