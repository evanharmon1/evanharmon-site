...
| `domain:{auth,brand,content,seo,hosting}` | humans or agents, at triage | humans, `gh issue list --label` | provisioned; inert | durable classification; the label family is the only surface — there is no paired project field |
| `area:{ci,docs,deps,build,tests,tasks,release,devcontainer,pm,skills,gauntlet,components,pages,layouts,styles,blog,cms,assets,design-system,e2e,lighthouse,hosting,astro,agents-md,specs,forms,content,brand-kit}` | humans or agents, at triage | humans, `gh issue list --label` | provisioned; inert | durable classification; area = solution space, domain = problem space, layer = stack slice |
